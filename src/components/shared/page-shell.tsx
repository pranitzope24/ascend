...
export function PageHeader({ actions, className, description, eyebrow, title }: PageHeaderProps) {
  return (
    <header
      className={cn(
        "mb-6 flex min-w-0 flex-wrap items-start gap-x-3 gap-y-4 sm:mb-8 sm:gap-6",
        className
      )}
    >
      <div className="min-w-[200px] flex-1">
        {eyebrow && <p className="text-primary mb-1 text-sm font-medium">{eyebrow}</p>}
        <h1 className="text-2xl font-semibold tracking-tight text-balance sm:text-3xl">{title}</h1>
        {description && (
          <p className="text-muted-foreground mt-1.5 max-w-2xl text-sm text-pretty sm:mt-2">
            {description}
          </p>
        )}
      </div>
