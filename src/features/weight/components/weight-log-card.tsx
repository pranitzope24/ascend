    ...
    <div className="flex flex-col gap-2 p-4 border rounded-xl bg-card">
      <div className="flex justify-between items-start">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" onClick={() => onEdit(log)} className="h-8 w-8 text-muted-foreground">
            <Edit2 className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="icon" onClick={handleDelete} disabled={isDeleting} className="h-8 w-8 text-destructive hover:text-destructive hover:bg-destructive/10">
            <Trash2 className="h-4 w-4" />
          </Button>
        </div>
      </div>
      {log.note && (
        <div className="text-sm pt-2 text-foreground/80 whitespace-pre-wrap">
          {log.note}
        </div>
      )}
    </div>
