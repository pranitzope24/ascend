      ...
      <div className="flex flex-col h-full space-y-4 pb-4">
        ...
        <div className="flex-1 overflow-y-auto space-y-6">
          {isLoading ? (
            ...
            Object.entries(groupedExercises).map(([initial, list]) => (
              <div key={initial} className="space-y-1">
                <h4 className="font-semibold text-muted-foreground px-2 py-1 bg-muted/30 sticky top-0 backdrop-blur-sm z-10">
                  {initial}
                </h4>
                {list.map((ex) => (
                  <Button 
